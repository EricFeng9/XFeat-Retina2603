...
def get_default_config():
    ...
    conf.TRAINER.CANONICAL_BS = 2   # LighterGlue 训练显存大，默认 2；显存充足可改为 4
    ...
    conf.TRAINER.TRUE_BATCH_SIZE = 1
    ...
    conf.XFEAT = {
        ...
        'top_k': 2048,   # 训练时降低以减轻 LightGlue O(N²) 显存；推理可改回 4096
        'detection_threshold': 0.05,
    }

    # 官方轻量版 LightGlue (LighterGlue)：显存与关键点数 N 的平方相关，训练时建议 top_k≤2048、batch≤2
    conf.LIGHTERGLUE = {
...
class RealDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # LighterGlue 输出 matches0 为 Image0(Fix) -> Image1(Moving) 的匹配索引
        # 所以这里取逆
        try:
...
class PL_XFeat_Gen(pl.LightningModule):
    ...
    def _compute_gt_matches(self, kpts0, kpts1, T_0to1, valid_mask0=None, valid_mask1=None, dist_th=3.0):
        ...
        T_0to1: [B, 3, 3]，image0 -> image1 的变换
        ...
        for b in range(B):
            ...
            kp0_h = torch.cat([kp0, ones], dim=-1)
            kp0_w_h = kp0_h @ T_0to1[b].transpose(0, 1)
            kp0_w = kp0_w_h[:, :2] / (kp0_w_h[:, 2:3] + 1e-8)

            dist = torch.cdist(kp0_w, kp1)
            min_dist, idx = torch.min(dist, dim=-1)
            keep = min_dist < dist_th
            idx = torch.where(keep, idx, idx.new_full(idx.shape, -1))

            global_idx1 = torch.nonzero(m1, as_tuple=False).squeeze(1)
            mapped = idx.clone()
            ok = mapped > -1
            if ok.any():
                mapped[ok] = global_idx1[mapped[ok]]

            global_idx0 = torch.nonzero(m0, as_tuple=False).squeeze(1)
...
class MultimodalValidationCallback(Callback):
    def __init__(self, args):
        ...
        self.args = args
        self.best_val = -1.0
        self.result_dir = Path(f"/data/student/Fengjunming/diffusion_registration/XFeat/results/xfeat_gen/{args.name}")
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="XFeat 训练脚本")
    parser.add_argument('--name', '-n', type=str, default='260303_xfeat', help='训练名称')
    parser.add_argument('--batch_size', type=int, default=1, help='LighterGlue 显存消耗大，建议默认 1；OOM 再降')
    parser.add_argument('--top_k', type=int, default=None, help='XFeat 每图最大关键点数，默认用 config；OOM 时建议 1024 或 2048')
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("【LighterGlue 训练】官方轻量版 LightGlue (xfeat-lighterglue.pt)")
    logger.info("=" * 80)
