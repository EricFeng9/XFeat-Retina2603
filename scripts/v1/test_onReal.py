...
# 导入 GlueStick 相关模块（与 train_onReal.py 对齐）
...
# 导入数据集（与 train_onReal.py 对齐）
from dataset.CFFA.cffa_dataset import CFFADataset

# 导入指标计算模块（与 train_onReal.py 对齐，使用 v2）
...
# ==========================================
# 辅助类: RealDatasetWrapper (格式转换，与 train_onReal.py 一致)
# ==========================================
...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        # 测试阶段使用真实数据集（CFFA），与 train_onReal.py 验证集对齐
        script_dir = Path(__file__).parent.parent.parent
        
        # 测试集：真实数据（路径与 train_onReal.py 一致）
...
# ==========================================
# 核心模型: PL_GlueStick_Real（与 train_onReal.py 完全对齐）
# ==========================================
class PL_XFeat_Real(pl.LightningModule):
    """XFeat 的 PyTorch Lightning 封装（用于真实数据训练，测试用）"""
    ...
    def test_step(self, batch, batch_idx):
        """测试步骤（与 train_onReal.py 的 validation_step 完全对齐）"""
        outputs = self(batch)
        ...
        # 【修复】与 train_onReal.py 对齐：逐batch计算AUC并log
...
class TestCallback(Callback):
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        # 【修复】从 trainer.callback_metrics 获取 AUC（与 train_onReal.py 对齐）
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="XFeat CFFA Real-Data Testing (aligned with train_onReal.py)")
...
def main():
    args = parse_args()
    args.mode = 'cffa'  # 固定为 CFFA 模式（与 train_onReal.py 对齐）
    ...
    model = PL_XFeat_Real.load_from_checkpoint(
        str(ckpt_path),
        config=config,
