...
class PL_XFeat_Gen(pl.LightningModule):
    """XFeat 的 PyTorch Lightning 封装（用于生成数据训练）"""
    ...
    def _is_on_vessel(self, kpt, vessel_mask):
        """检查关键点是否在血管上"""
        # vessel_mask: [1, H, W] 或 [H, W]
        if vessel_mask.dim() == 3:
            mask = vessel_mask[0]  # [H, W]
        else:
            mask = vessel_mask
        
        H, W = mask.shape
        x = int(torch.clamp(kpt[0], 0, W - 1).item())
        y = int(torch.clamp(kpt[1], 0, H - 1).item())
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="XFeat Gen-Data Training with 260227_2_v29_2_1 Dataset")
    parser.add_argument('--name', '-n', type=str, default='xfeat_gen_baseline', help='训练名称')
...
def main():
    ...
    # 初始化模型
    model = PL_XFeat_Gen(config, result_dir=str(result_dir))
    ...
    logger.info(f"开始 XFeat 混合训练 (训练集: 260227_2_v29_2_1 生成数据 | 验证集: CFFA 真实数据): {args.name}")
